...
export default async function KategorijasSludinajumi({ params }: Props) {
  ...
  return (
    <div className="min-h-screen bg-white">
      {/* Saglabā identisku navigāciju un struktūru */}
              ...
              <div key={item.id} className="group bg-white border rounded-xl overflow-hidden shadow-md hover:shadow-xl transition-all hover:-translate-y-1">
                <Link href={`/sludinajums/${item.id}`} className="block">
                  <div className="relative h-48 bg-gray-100 overflow-hidden">
                    <img src={item.image_public_urls?.[0] || '/placeholder-car.jpg'} alt={item.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
                  </div>
                  <div className="p-5">
                    <h3 className="font-bold text-lg mb-2 line-clamp-2 leading-tight">{item.title}</h3>
                    <p className="text-sm text-gray-600 mb-4 line-clamp-2">{item.description}</p>
                    <div className="flex justify-between items-center mb-4">
                      <span className="text-2xl font-bold text-blue-600">{item.price ? `${item.price}€` : 'Cena vienojoties'}</span>
                ...
                <div className="px-5 pb-5">
                  <div className="flex gap-2">
                    <Link href={`/sludinajums/${item.id}`} className="flex-1 bg-gray-100 text-gray-800 py-3 px-4 rounded-lg text-center font-medium hover:bg-gray-200">Skatīt</Link>
                  </div>
                </div>
              </div>
            ))}
