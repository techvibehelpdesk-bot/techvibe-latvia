...
export default function Moto() {
  ...
  const fetchSludinajumi = async () => {
    try {
      const { data, error } = await supabase
        .from('sludinajumi')
        .select('*')
        .eq('category', 'moto')  // 🏍️ MOTO KATEGORIJA
        .eq('status', 'publicēts')
  ...
  if (loading) {
    return <div style={{minHeight: '100vh', background: 'linear-gradient(to bottom right, #f97316, #ea580c)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.25rem', color: 'white'}}>🏍️ Ielādē moto sludinājumus...</div>;
  }

  return (
    <>
      <Head>
        <title>🏍️ Moto - TechVibe.lv</title>
      </Head>
      <div style={{minHeight: '100vh', background: 'linear-gradient(to bottom right, #f97316, #ea580c)', padding: '2rem 1rem'}}>
        <div style={{maxWidth: '1200px', margin: '0 auto'}}>
          <div style={{marginBottom: '2rem', display: 'flex', flexDirection: 'column', alignItems: 'flex-start'}}>
            <h1 style={{fontSize: '2.5rem', fontWeight: 'bold', color: 'white', marginBottom: '0.5rem'}}>🏍️ Moto transports</h1>
            <p style={{fontSize: '1.25rem', color: 'rgba(255,255,255,0.9)', marginBottom: '1rem'}}>
              {sludinajumi.length} moto sludinājumi Rīgai un Latvijai
            </p>
            <Link 
              href="/ievietot" 
              style={{
                background: 'white', color: '#ea580c', padding: '0.75rem 2rem', borderRadius: '0.75rem', 
          ...
          {sludinajumi.length === 0 ? (
            <div style={{textAlign: 'center', padding: '5rem 1rem'}}>
              ...
              <Link href="/ievietot" style={{background: 'white', color: '#ea580c', padding: '0.75rem 2rem', borderRadius: '0.75rem', fontWeight: '600', textDecoration: 'none'}}>
                Būt pirmais!
              </Link>
            </div>
          ) : (
                  ...
                  <div style={{height: '12rem', background: 'linear-gradient(to right, #f59e0b, #d97706)', position: 'relative', overflow: 'hidden'}}>
                    {sludinajums.images && sludinajums.images[0] ? (
                      ...
                      <div style={{width: '100%', height: '100%', background: '#fef3c7', display: 'flex', alignItems: 'center', justifyContent: 'center'}}>
                        <span style={{color: '#92400e', fontSize: '1.125rem'}}>🏍️ Nav bildes</span>
                      </div>
                    )}
                  </div>
