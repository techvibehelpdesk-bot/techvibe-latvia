...
export default function Berniem() {
  ...
  const fetchSludinajumi = async () => {
    try {
      const { data, error } = await supabase
        .from('sludinajumi')
        .select('*')
        .eq('category', 'berniem')  // 👶 BĒRNIEM KATEGORIJA
        .eq('status', 'publicēts')
      ...
      console.error('Kļūda ielādējot bērnu sludinājumus:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  if (loading) {
    return <div style={{minHeight: '100vh', background: 'linear-gradient(to bottom right, #fefce8, #fef9c3)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.25rem'}}>👶 Ielādē bērnu sludinājumus...</div>;
  }

  return (
    <>
      <Head>
        <title>👶 Bērniem - TechVibe.lv</title>
      </Head>
      <div style={{minHeight: '100vh', background: 'linear-gradient(to bottom right, #fefce8, #fef9c3)', padding: '2rem 1rem'}}>
        <div style={{maxWidth: '1200px', margin: '0 auto'}}>
          <div style={{marginBottom: '2rem', display: 'flex', flexDirection: 'column', alignItems: 'flex-start'}}>
            <h1 style={{fontSize: '2.5rem', fontWeight: 'bold', color: '#1f2937', marginBottom: '0.5rem'}}>👶 Bērniem</h1>
            <p style={{fontSize: '1.25rem', color: '#6b7280', marginBottom: '1rem'}}>
              {sludinajumi.length} sludinājumi bērniem un ģimenei
            </p>
            <Link 
              href="/ievietot" 
              style={{
                ...
                >
                  <div style={{height: '12rem', background: 'linear-gradient(to right, #f59e0b, #d97706)', position: 'relative', overflow: 'hidden'}}>
                    {sludinajums.images && sludinajums.images[0] ? (
                      ...
                      <div style={{width: '100%', height: '100%', background: '#fde68a', display: 'flex', alignItems: 'center', justifyContent: 'center'}}>
