...
export default function Darbs() {
  ...
  const fetchSludinajumi = async () => {
    try {
      const { data, error } = await supabase
        .from('sludinajumi')
        .select('*')
        .eq('category', 'darbs')  // 💼 DARBS KATEGORIJA
        .eq('status', 'publicēts')
      ...
      console.error('Kļūda ielādējot darba sludinājumus:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  if (loading) {
    return <div style={{minHeight: '100vh', background: 'linear-gradient(to bottom right, #ecfdf5, #d1fae5)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.25rem'}}>💼 Ielādē darba sludinājumus...</div>;
  }

  return (
    <>
      <Head>
        <title>💼 Darbs - TechVibe.lv</title>
      </Head>
      <div style={{minHeight: '100vh', background: 'linear-gradient(to bottom right, #ecfdf5, #d1fae5)', padding: '2rem 1rem'}}>
        <div style={{maxWidth: '1200px', margin: '0 auto'}}>
          <div style={{marginBottom: '2rem', display: 'flex', flexDirection: 'column', alignItems: 'flex-start'}}>
            <h1 style={{fontSize: '2.5rem', fontWeight: 'bold', color: '#1f2937', marginBottom: '0.5rem'}}>💼 Darbs</h1>
            <p style={{fontSize: '1.25rem', color: '#6b7280', marginBottom: '1rem'}}>
              {sludinajumi.length} vakances un darba piedāvājumi Rīgai
            </p>
            <Link 
              href="/ievietot" 
              style={{
              ...
              {sludinajumi.map((sludinajums) => (
                <div 
                  ...
                  style={{
                    ...
                    transition: 'all 0.3s', overflow: 'hidden', borderLeft: '4px solid #10b981'
                  }}
                  onMouseEnter={(e) => {
                ...
                >
                  <div style={{height: '10rem', background: 'linear-gradient(to right, #10b981, #059669)', position: 'relative', overflow: 'hidden'}}>
                    {sludinajums.images && sludinajums.images[0] ? (
                      ...
                      <div style={{width: '100%', height: '100%', background: '#d1fae5', display: 'flex', alignItems: 'center', justifyContent: 'center'}}>
                        <span style={{color: '#065f46', fontSize: '1.125rem'}}>💼 Darba sludinājums</span>
                      </div>
                    )}
                  </div>
