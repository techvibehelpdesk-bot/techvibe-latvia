...
export default function Buvmateriali() {
  ...
  const fetchSludinajumi = async () => {
    try {
      const { data, error } = await supabase
        .from('sludinajumi')
        .select('*')
        .eq('category', 'buvmateriali')  // 🔨 BŪVMATERIĀĻI KATEGORIJA
        .eq('status', 'publicēts')
      ...
      console.error('Kļūda ielādējot būvmateriālu sludinājumus:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  if (loading) {
    return <div style={{minHeight: '100vh', background: 'linear-gradient(to bottom right, #f3f4f6, #e5e7eb)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.25rem'}}>🔨 Ielādē būvmateriālu sludinājumus...</div>;
  }

  return (
    <>
      <Head>
        <title>🔨 Būvmateriāli - TechVibe.lv</title>
      </Head>
      <div style={{minHeight: '100vh', background: 'linear-gradient(to bottom right, #f3f4f6, #e5e7eb)', padding: '2rem 1rem'}}>
        <div style={{maxWidth: '1200px', margin: '0 auto'}}>
          <div style={{marginBottom: '2rem', display: 'flex', flexDirection: 'column', alignItems: 'flex-start'}}>
            <h1 style={{fontSize: '2.5rem', fontWeight: 'bold', color: '#1f2937', marginBottom: '0.5rem'}}>🔨 Būvmateriāli</h1>
            <p style={{fontSize: '1.25rem', color: '#6b7280', marginBottom: '1rem'}}>
              {sludinajumi.length} būvmateriālu sludinājumi Rīgai
            </p>
            <Link 
              href="/ievietot" 
              style={{
            ...
            >
              ➕ Ievietot būvmateriālu sludinājumu
            </Link>
          </div>
          ...
          {sludinajumi.length === 0 ? (
            <div style={{textAlign: 'center', padding: '5rem 1rem'}}>
              ...
              <h2 style={{fontSize: '2rem', fontWeight: 'bold', color: '#1f2937', marginBottom: '1rem'}}>
                Vēl nav būvmateriālu sludinājumu
              </h2>
            ...
            <div style={{display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '1.5rem'}}>
              {sludinajumi.map((sludinajums) => (
                ...
                >
                  <div style={{height: '12rem', background: 'linear-gradient(to right, #1f2937, #374151)', position: 'relative', overflow: 'hidden'}}>
