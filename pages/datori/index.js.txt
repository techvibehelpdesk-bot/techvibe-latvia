...
export default function Datori() {
  ...
  const fetchSludinajumi = async () => {
    try {
      const { data, error } = await supabase
        .from('sludinajumi')
        .select('*')
        .eq('category', 'datori')  // 💻 DATORI KATEGORIJA
        .eq('status', 'publicēts')
      ...
      console.error('Kļūda ielādējot datoru sludinājumus:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  if (loading) {
    return <div style={{minHeight: '100vh', background: 'linear-gradient(to bottom right, #ecfdf5, #6ee7b7)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.25rem'}}>💻 Ielādē datoru sludinājumus...</div>;
  }

  return (
    <>
      <Head>
        <title>💻 Datori - TechVibe.lv</title>
      </Head>
      <div style={{minHeight: '100vh', background: 'linear-gradient(to bottom right, #ecfdf5, #6ee7b7)', padding: '2rem 1rem'}}>
        <div style={{maxWidth: '1200px', margin: '0 auto'}}>
          <div style={{marginBottom: '2rem', display: 'flex', flexDirection: 'column', alignItems: 'flex-start'}}>
            <h1 style={{fontSize: '2.5rem', fontWeight: 'bold', color: '#1f2937', marginBottom: '0.5rem'}}>💻 Datori un tehnika</h1>
            <p style={{fontSize: '1.25rem', color: '#6b7280', marginBottom: '1rem'}}>
              {sludinajumi.length} datoru un tehnikas sludinājumi
            </p>
            <Link 
              href="/ievietot" 
              style={{
                  ...
                  <div style={{height: '12rem', background: 'linear-gradient(to right, #3b82f6, #1d4ed8)', position: 'relative', overflow: 'hidden'}}>
                    {sludinajums.images && sludinajums.images[0] ? (
                      ...
                      <div style={{width: '100%', height: '100%', background: '#dbeafe', display: 'flex', alignItems: 'center', justifyContent: 'center'}}>
                        <span style={{color: '#1e40af', fontSize: '1.125rem'}}>Nav bildes</span>
                      </div>
                    )}
                  </div>
