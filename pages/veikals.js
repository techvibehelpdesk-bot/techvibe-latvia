...
// Izveidojam savienojumu ar Supabase
...
export default function Veikals() {
  ...
  useEffect(() => {
    async function fetchItems() {
      // Šeit mēs prasām Supabase iedot visus sludinājumus
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {items.map((item) => (
              <div key={item.id} className="bg-white p-4 rounded-lg shadow-md hover:shadow-lg transition">
                {/* Ja ir bilde, parādam to. Ja nav, rādam pelēku kastīti */}
                {item.image_url ? (
                  <img 
                    src={item.image_url} 
                    alt={item.title} 
