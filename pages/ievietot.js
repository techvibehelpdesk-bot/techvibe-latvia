// pages/ievietot.js - SS.COM PRO ar kategorijām + BILDES + Contact
...
export default function IevietotSludinajumu() {
  ...
  const handleSubmit = async (e) => {
    ...
    // Upload bildes uz Storage
    if (images.length > 0) {
      const file = images[0]; // Pirmā bilde kā galvenā
      const fileExt = file.name.split('.').pop();
      const fileName = `${Date.now()}.${fileExt}`;
      
      const { data } = await supabase.storage
        .from('sludinajumi') // Izveido bucket!
        .upload(fileName, file, { upsert: true });
      
      imageUrl = supabase.storage.from('sludinajumi').getPublicUrl(fileName).data.publicUrl;
    }
    ...
    try {
      ...
      alert('🚀 Sludinājums + bildes nosūtītas! Gaidi apstiprinājumu.');
      e.target.reset();
      setImages([]);
    } catch (error) {
        ...
        <div className="max-w-4xl mx-auto px-4">
          ...
          <div className="bg-white shadow-xl rounded-2xl p-8 mb-8">
            ...
            <div className="mb-8 p-6 bg-gradient-to-r from-indigo-50 to-purple-50 rounded-xl border-2 border-dashed border-indigo-200">
              <label className="block text-lg font-semibold mb-4 text-gray-800">1. Izvēlies kategoriju</label>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                ...
                <input id="virsraksts" name="virsraksts" type="text" className="w-full border border-gray-300 rounded-xl px-4 py-3 focus:ring-2 focus:ring-purple-500" placeholder="iPhone 15 Pro Max" required />
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <input id="cena" name="cena" type="number" className="w-full border border-gray-300 rounded-xl px-4 py-3 focus:ring-2 focus:ring-purple-500" min="0" step="0.01" />
                </div>
                <div>
                  <label className="block text-sm font-semibold mb-2 text-gray-700" htmlFor="kontakts">Tālrunis/e-pasts (privāts)</label>
              ...
              {/* BILŽU UPLOAD – JAUNS! */}
              <div>
                <label className="block text-sm font-semibold mb-2 text-gray-700">📸 Bildes (līdz 3 gab.)</label>
                <input
                  type="file"
                  multiple
                  accept="image/*"
                ...
                />
                {images.length > 0 && (
                  <p className="text-sm text-green-600 mt-2">✅ Izvēlētas {images.length} bilde(s)</p>
                )}
              </div>
              ...
              >
                {loading ? '🚀 Nosūta ar bildēm...' : '🚀 Publicēt sludinājumu + bildes'}
              </button>
